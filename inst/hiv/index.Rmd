---
title: "<a href='http://www.ici3d.org' alt = 'ICI3D'><img src='http://www.ici3d.org/assets/img/logo/ici3d-logo-nav.png' title='SACEMA' width = '300'></a><br><br>HIV in Harare tutorial"
...
hivdat <- system.file("hiv", "HIVtutorial.Rdata", package="DAIDD")
endYear <- 2025
```
...
- Where in the model code does it show that the instantaneous rate of change of the susceptible compartment is calculated as the total birth rate, minus the background mortality rate, minus incidence? How would you express this as a differential equation?
...
- Click on the "Show model output" button to the right below.
- The model's prediction for the trajectory of HIV prevalence is now shown in dark green on the plot below. Mortality (blue) and incidence (red) trajectories are also shown.
- The small graph below shows the intervention as a function of time. You can vary the intervention paramters to view their effect.
...
renderPlot({
  ff <- Vectorize(function(tt,cMax,cRate,cHalf){
    ...
    par(mar=c(6,5,4,1),cex.axis=1.5,cex.lab=1.5,cex.main=1.5,lwd=3)
    curve(ff(x,input$cMax4,input$cRate4,input$cHalf4)
          , xlim = c(startYear,endYear)
          , ylim = c(0,1)
    ...
    if(input$cMax4 == 0|input$cRate4 == 0){
      mtext('No intervention implemented.',3,-1,col='dark grey',cex=2)
    }
},width=580)
...
renderPlot({
  ...
  with(.selectSite(input$useSite),{
    if(input$runButton5==0){
      ...
      inc <- inc * exp(-input$q5*0.018*traj$I4/(traj$S+traj$I))
      par(mar=c(6,5,4,4),cex.axis=1.5,cex.lab=1.5,cex.main=1.5,lwd=4)
      plot(year,prevalence
           , pch = 16
           , cex = 1.8
...
sidebarPanel(
  ...
  br(),
  sliderInput('q5',
              div(HTML('Effect of behavior change in response to mortality:')),
              min = 0,
              max = 450,
              value = 0,
              step = 1),
