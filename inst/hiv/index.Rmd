...
```{r, echo=FALSE, message=FALSE}
# renderPlot({
#   ff <- Vectorize(function(tt){
#     input$cMax4/(1+exp(-input$cRate4*(tt-input$cHalf)))
#   })
#   if(input$runButton4==0){
#     par(mar=c(6,5,4,0),cex.axis=1.5,cex.lab=1.5,cex.main=1.5,lwd=3)
#     curve(ff(x)
...
#       mtext('No intervention implemented.',3,-1,col='cyan')
#     }
#   }
# },width=580)
...
sidebarPanel(
  ...
  sliderInput('lambda5',
              ...
              step = 0.05),
    sliderInput('p05',
              'Natural log of the intial infection prevalence ($p_0$:',
              min = -10,
              max = -3,
              value = -7,
              step = 0.5),
