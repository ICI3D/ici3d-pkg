...
quiz(
  question_radio(
    ...
    answer("Important distinction(s).", message = "No; while the precise outcomes for the two populations are not identical, they have similar end dates, peak incidence (both timing and intensity), and overall totals.")
  )
)
```
...
quiz(
  question_radio(
    ...
    answer("Important distinction(s).", message = "No; while the precise outcomes for the two years are not identical, both years, for both populations, have similar end dates, peak incidence (both timing and intensity), and overall totals.")
  )
)
```
...
dynfever_plot_heatmap(dynfever_DAIDD_distros[vax_pop == pop])
```
...
quiz(
  ...
  question_freeresponse(
    "Based on what you think of the model world: What impact do you expect vaccinating 50% of dogs to have on the human cases? What about 50% of humans, on dog cases?  Why?",
    "Take note of your answers; we will discuss them among the full group."
  )
)
```
